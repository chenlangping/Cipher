...
public class Playfair extends JDialog implements ActionListener {
    ...
    public Playfair() {
        ...
        ok1.addActionListener(new ActionListener() { // 确定事件处理
            public void actionPerformed(ActionEvent Event) {
                plaintext = plaintextinput.getText().replaceAll(" ", "");// 明文,去掉空格
                ciphertext = ciphertextinput.getText();
                key = keyinput.getText().replaceAll(" ", "");// 密钥，去掉空格
                char replace = 'x'; // 填充字符
                if (encryption.isSelected() == true) {
                    if (plaintext.equals("") && key.equals("")) {
                        ...
                        // 执行加密算法
                        char[] ciphertext = new Playfairalgorithm().encode(plaintext.toLowerCase().toCharArray(),
                                key.toLowerCase().toCharArray(), replace);
                        ciphertextinput.setText(new String(ciphertext));
                    }
                    ...
                    if (ciphertext.equals("") && key.equals("")) {
                        ...
                        // 输入密文和密钥解密
                        plaintextinput.setText(new String(plaintext1));
                    }
                }
            }
        });
                ...
                if (fileencryption.isSelected() == true) {
                    ...
                    if (fileplaintextinput.getText().equals("") && fileciphertextinput.getText().equals("")
                        ...
                        if (fileplaintextlegal && filekeylegal) {
                            // 执行加密算法
                            char[] ciphertext = new Playfairalgorithm().encode(fileplaintext.toLowerCase().toCharArray(),
                                    filekey.toLowerCase().toCharArray(), replace);
                            fileciphertext = new String(ciphertext);
                            ...
                            JOptionPane.showMessageDialog(jf, "在明文中检测到非法字符！");
                        } else if (fileplaintextlegal && !filekeylegal) {
                            JOptionPane.showMessageDialog(jf, "在密钥中检测到非法字符！");
                        } else JOptionPane.showMessageDialog(jf, "在明文和密钥中都检测到非法字符！");
                    ...
                    if (fileplaintextinput.getText().equals("") && fileciphertextinput.getText().equals("")
                        ...
                        if (fileciphertextlegal && filekeylegal) {
                            char[] a = new Playfairalgorithm().decode(fileciphertext.toCharArray(),
                                    filekey.toLowerCase().toCharArray()); // 输入密文和密钥解密
                            fileplaintext = new String(a);
                            ...
                            JOptionPane.showMessageDialog(jf, "在密文中检测到非法字符！");
                        } else if (fileciphertextlegal && !filekeylegal) {
                            JOptionPane.showMessageDialog(jf, "在密钥中检测到非法字符！");
                        } else JOptionPane.showMessageDialog(jf, "在密文和密钥中都检测到非法字符！");
